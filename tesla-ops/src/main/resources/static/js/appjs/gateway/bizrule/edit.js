...
$(document).ready(function() {
    ...
    $(".choose").each(function() {
      $(this).bind("click", function() {
        ...
        if (!hasAdded) {
          ...
          var nextTab = $('.nav-tabs').find('li').size() + 1;
          var li = '<li><a href="#' + userFilter + '" data-toggle="tab">' + nextTab + "#" + userFilter + '</a></li>';
          var content = '<div class="tab-pane" id="' + userFilter + '"></div>';
          $('.nav-tabs').append(li);
          $('.tab-content').append(content);
          $('.nav-tabs').find('a:last').tab('show');
          if (userFilter == 'DataMappingRequestFilter' || userFilter == 'DataMappingHttpResponseFilter') {
            $('#fremarkderSection').tmpl({
              filterIndex: nextTab - 1,
  ...
  var selectUrl = function() {
    ...
    var apiId = $("#selectApiId").val();
    var apiGroupId = $("#selectApiGroupId").val();
